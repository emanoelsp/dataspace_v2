...
export default function FederationDetailsPage() {
    ...
    useEffect(() => {
        const loadOwnerConnector = async () => {
            ...
            const federationConnector = {
                participantId: typeof federation.participantId === "string" ? federation.participantId : "",
                connectorDspBaseUrl: typeof federation.connectorDspBaseUrl === "string" ? federation.connectorDspBaseUrl : "",
                connectorManagementBaseUrl:
                    typeof federation.connectorManagementBaseUrl === "string" ? federation.connectorManagementBaseUrl : "",
                federatedCatalogUrl: typeof federation.federatedCatalogUrl === "string" ? federation.federatedCatalogUrl : "",
            }

            if (federationConnector.participantId && federationConnector.connectorDspBaseUrl) {
                setOwnerConnector(federationConnector)
                return
            }
    ...
    const publishFederationToCatalog = async () => {
        if (!federation || !canManageFederation) return
        const connectorParticipantId =
            (typeof federation.participantId === "string" ? federation.participantId : "") || profile?.participantId || ""
        const connectorDspBaseUrl =
            (typeof federation.connectorDspBaseUrl === "string" ? federation.connectorDspBaseUrl : "") || profile?.connectorDspBaseUrl || ""
        const federatedCatalogUrl =
            (typeof federation.federatedCatalogUrl === "string" ? federation.federatedCatalogUrl : "") || profile?.federatedCatalogUrl || ""

        if (!connectorParticipantId || !connectorDspBaseUrl) {
        ...
        try {
            ...
            await setDoc(doc(db, "catalogPublications", `federation-${federation.id}`), {
                ...
                connectorParticipantId: connectorParticipantId ?? federation.ownerId ?? "",
                dspEndpoint: connectorDspBaseUrl,
                catalogEndpoint: federatedCatalogUrl,
                published: true,
