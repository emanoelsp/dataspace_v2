...
export const CONNECTOR_CREDENTIAL_STATUSES = ["issued", "active", "rotated", "revoked", "expired"] as const

export type ConnectorCredentialStatus = (typeof CONNECTOR_CREDENTIAL_STATUSES)[number]

export const IDENTITY_TOKEN_STATUSES = ["issued", "active", "expired", "revoked", "rejected"] as const

export type IdentityTokenStatus = (typeof IDENTITY_TOKEN_STATUSES)[number]
...
export interface IdentityTrustLogRecord {
  connectorProfileId: string
  participantId: string
  targetParticipantId?: string
  identityTokenId?: string
  connectorConnectionId?: string
  event: "issued" | "verified" | "rejected" | "revoked" | "expired"
  detail?: string
  ownerId: string
  ownerEmail?: string
  recordedAt?: unknown
