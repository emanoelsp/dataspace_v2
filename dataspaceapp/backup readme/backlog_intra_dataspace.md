...
Este backlog detalha a trilha gradativa `0 -> 5` para evoluir o prototipo atual para um conector INTRA alinhado ao modelo documentado em `modelo_connector_intra.md` e aos requisitos descritos em `requisitos_software.md`.

## 0. Infraestrutura e Identity Trust

Objetivo: estabelecer a camada fundacional de confianca antes de liberar catalogo, membership, contrato ou consumo.

### 0.1 Colecoes novas ou especializadas

- `connectorProfiles`
  - desacopla a configuracao tecnica do conector do perfil humano
  - armazena `participantId`, `connectorDspBaseUrl`, `connectorManagementBaseUrl`, `federatedCatalogUrl`, `sidecarProtocol`, `sidecarEndpoint`
- `connectorCredentials`
  - armazena `clientId`, `clientSecretHash`, status, rotacao e expiracao
  - pode ser emitida para owner e client, conforme o conector registrado
- `identityTokens`
  - trilha de emissao, expiracao, revogacao e falha de autenticacao
  - registra `participantId`, `scope`, `issuedAt`, `expiresAt`, `status`
- `identityTrustLogs`
  - auditoria de autenticacao mutua entre conectores
  - registra sucesso, falha, revogacao e bloqueios por credencial invalida

### 0.2 Campos a reforcar nas entidades atuais
...
- sem `Identity Trust` ativo nao existe navegacao em catalogo, request de membership, negociacao de contrato nem emissao de token de acesso.

### 0.5 Criterio de aceite

- owner e client possuem conectores com identidade tecnica configurada;
- o client recebe credencial verificavel para autenticar seu conector;
- autenticacao valida gera token de identidade;
- autenticacao invalida retorna `401/403` e bloqueia o restante do fluxo.
