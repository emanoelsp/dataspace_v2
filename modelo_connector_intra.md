...
- `connectorProfiles`
  - opcional se quiser desacoplar `users` da configuracao tecnica do connector
...
3. Resolver/catalogar o owner via catalogo federado.
4. Explorar federacoes visiveis.
5. Entrar na federacao conforme o modo:
   - `self-service`: aceita e ativa;
   - `approval`: assina e aguarda aprovacao;
   - `invite-only`: recebe convite, assina e devolve.
6. Explorar assets da federacao.
7. Escolher `ContractOffer` do asset.
8. Negociar e assinar o `ContractAgreement`.
9. Iniciar `TransferProcess`.
10. Receber `EDR`/token e consumir na fonte.
